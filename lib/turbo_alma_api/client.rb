...
module TurboAlmaApi
  ...
  class Client
    ...
    def self.all_items_for(mms_id, username = nil)
      Bib::PennBibSet.new mms_id, username
    end
    ...
    def self.item_for(identifiers)
      ...
      TurboAlmaApi::Bib::PennItem.new parsed_response['item_data']
    end
    ...
    def self.api_get_request(url, headers = {})
      headers.merge! TurboAlmaApi::Client::DEFAULT_REQUEST_HEADERS
    ...
    def self.request_url(mms_id, holding_id, item_id)
      "#{TurboAlmaApi::Client::BASE_URL}/v1/bibs/#{mms_id}/holdings/#{holding_id}/items/#{item_id}/requests"
    end

    # @param [Hash] identifiers
    def self.item_url(identifiers)
      "#{TurboAlmaApi::Client::BASE_URL}/v1/bibs/#{identifiers[:mms_id]}/holdings/#{identifiers[:holding_id]}/items/#{identifiers[:item_pid]}"
    end
